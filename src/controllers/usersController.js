      ...
      .then(users => {
        ...
        if (bcrypt.compareSync(password, users.password)) {
          if (users.type === "Administrador") {
            ...
            res.cookie('admi', users.email, { maxAge: 60000 });
            db.Products.findAll()
              .then((product) => {
                return res.render('Administrador', { products: product })
              });
          } else {
            res.cookie('cliente', users.email, { maxAge: 60000 });
            db.Products.findAll()
              .then((product) => {
                return res.render('Cliente', { products: product })
              })
          }
        } else {
    ...
    db.Users.findByPk(id)
      .then(user => {
        if (req.cookies && req.cookies.admi) {
          ...
          return res.render('users/deletClient', { user: user });
        }
